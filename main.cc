...
int main()
{
	...
	//std::vector<std::unique_ptr<Woodsman>> woodsmen;
	...
	UiButton bt_add_villager(sf::Vector2f(400, 710), sf::Color::Yellow, "Villager");
	bt_add_villager.setScale(0.5f, 0.5f);
	bt_add_villager.callback_ = [&window, &map, &woodsmen]()
		{
			woodsmen.emplace_back(window.getSize().x / 2, window.getSize().y / 2, 100, map);
		};
	...
	while (window.isOpen())
	{
		...
		while (window.pollEvent(event))
		{
			...
			bt_add_villager.HandleEvent(event);
